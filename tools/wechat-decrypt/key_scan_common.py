...
def _secret_value(label, value):
    """Avoid printing raw decrypted keys unless explicitly requested."""
    if os.environ.get("WECHAT_DECRYPT_SHOW_SECRETS") == "1":
            ...
            if salt_hex in remaining_salts:
                ...
                for rel, path, sz, s, page1 in db_files:
                    if s == salt_hex and verify_enc_key(enc_key, page1):
                        ...
                        print_fn(f"\n  [FOUND] salt={salt_hex}")
                        print_fn(f"    {_secret_value('enc_key', enc_key_hex)}")
